...
class Oracle:
    def __init__(self, oracle_name, output_dir, mol_buffer={}):
        self.name = None
        self.evaluator = tdc.Oracle(oracle_name)
        self.task_label = oracle_name
    ...
    def score_smi(self, smi):
        ...
        if mol is None or len(smi) == 0:
            ...
            if smi in self.mol_buffer:
                base_smi = smi  # 保留原始值用于计算
                suffix = 1  # 后缀计数器

                # 循环生成唯一键
                while smi in self.mol_buffer:
                    smi = f"{base_smi}_{suffix}"
                    suffix += 1

                # 始终使用原始 SMILES 计算评估值
                self.mol_buffer[smi] = [
                    float(self.evaluator(base_smi)),  # 用原始 SMILES 计算结果
                    len(self.mol_buffer) + 1  # 索引保持递增
                ]
            else:
