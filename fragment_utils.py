...
def reconstruct_decoration(frags, reverse=False):
    if len(frags) == 1:
        return strip_dummy_atoms(frags[0]), frags
...
def reconstruct_d(frags, reverse=False, scaffold=False):
    if len(frags) == 1:
        return strip_dummy_atoms(frags[0]), frags

    if count_dummies(frags[0]) != 1 and scaffold== False:
        return None, None
    ...
    if scaffold:
        sum = 0
        if count_dummies(frags[1]) == 2:
            smile = Chem.MolToSmiles(frags[1])
            for i in range(len(smile)):
                ...
                if (sum == 2):
                    pos = i
                    break
            smile = smile[:pos] + '[H]' + smile[pos+1:]
            frags[1] = Chem.MolFromSmiles(smile)
        mol = join_molecules(frags[0], frags[1])
    else:
        mol = join_molecules(frags[0], frags[1])
        for i, frag in enumerate(frags[2:]):
...
def reconstruct_scaffold1(frags, reverse=False, scaffold=False):
    if len(frags) == 1:
        return strip_dummy_atoms(frags[0]), frags
